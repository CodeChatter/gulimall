...
public class SeckillServiceImpl implements SeckillService {
        ...
        sessions.stream().forEach(session -> {
            ...
            session.getRelationSkus().stream().forEach(seckillSkuVo -> {
                ...
                if (!operations.hasKey(redisKey)) {
                    ...
                    //4、设置商品的随机码（防止恶意攻击）
                    // eg： /skuId=1 未到时间就遭到请求攻击或者开发内部知道提前抢
                    //    加了随机码 /skuId=1&key=yujkjk 未到时间随机码未出现，请求不了
     ...
     * 获取到当前可以参加秒杀商品的信息
     *
     * blockHandler方法必须和当前方法的返回值相同
     * @return
     */
    // blockHandler函数会在原方法被限流/降级/系统保护的时候调用，而fallback函数针对所有类型的异常。
